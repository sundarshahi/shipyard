...
Read Phase 1 outputs from `Shipyard/security-engineer/threat-model/` before beginning. The STRIDE analysis and attack surface map tell you WHERE to focus. Start with endpoints and code paths that scored Critical or High in the threat matrix.

## Inputs

- Phase 1 threat model -- `Shipyard/security-engineer/threat-model/`
...
Run ONLY if LLM/ML usage is detected (scan for `openai`, `anthropic`, `langchain`, `transformers`, `torch`, `tensorflow` imports — the same signal the orchestrator uses to auto-enable the data-scientist; see shipyard/SKILL.md "Conditional Tasks"). Each finding needs a `file:line` evidence pointer:
...
Write all outputs to `Shipyard/security-engineer/code-audit/`:
