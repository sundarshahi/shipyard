...
Grep <main entry files> for top-level declarations   # Architecture skeleton
```
...
Trace a user action end-to-end: HTTP request -> handler -> service -> repository -> database. Use Grep to trace the call chain without reading every file.
...
Write to `Shipyard/polymath/context/repo-map.md`:
